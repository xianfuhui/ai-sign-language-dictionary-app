...
const getListFavorite = async (req, res) => {
    ...
    try {
        const filtered_vocabulary_user = await VocabularyUser.find({ user: userId })
            .populate({
                path: 'vocabulary',
                match: { vocabulary_recommendation_status: { $in: [1, 3] } } 
            });

        const list_vocabulary_user = filtered_vocabulary_user.filter(v => v.vocabulary !== null);
        ...
        res.render('user/user_ListFavoriteVocabularyUser', { list_vocabulary_user });
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
};

// Lấy thông tin tất cả từ vựng đề xuất
const getListCustom = async (req, res) => {
    const userId = req.session.user._id;
    ...
    try {
        const filtered_vocabulary_user = await VocabularyUser.find({ user: userId })
            .populate({
                path: 'vocabulary',
                match: { vocabulary_recommendation_status: 2 } 
            });
        ...
        res.render('user/user_ListCustomVocabularyUser', { list_vocabulary_user });
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
};
...
const getListPractice = async (req, res) => {
    ...
    try {
        ...
        const list_practice = filtered_vocabulary_user.filter(v => v.vocabulary !== null);
...
const getListPracticeAI = async (req, res) => {
    ...
    try {
        //Hiển thị từ vựng yêu thích người dùng đăng nhập trong phiên session
        const filtered_vocabulary_user = await VocabularyUser.find({ user: userId })
            .populate({
                path: 'vocabulary',
                //Chỉ hiển thị từ vựng có trạng thái duyệt từ Admin (1 là có sẵn, 3 là người dùng tạo từ vựng được Admin duyệt)
                match: { vocabulary_recommendation_status: { $in: [1, 3] } } 
            });
